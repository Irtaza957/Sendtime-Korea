...
const ZoomBridge = () => {
  ...
  const { showModal } = useNestedModal({
    type: 'alert',
    title: 'Zoom 오류',
    description: `Zoom 내부 오류가 발생했습니다. 
    새로고침 후 다시 시도해주세요.`,
  });

  const { mutate: getZoomIntegration } = useMutation(
    (params: GetZoomIntegrationParams) =>
      externalServicesAPI.getZoomIntegration(params),
    {
      onSuccess: () => {
