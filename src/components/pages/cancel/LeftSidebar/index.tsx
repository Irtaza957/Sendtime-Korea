...
const LeftSidePreview = ({ pageInfo, timezone }: LeftSidePreviewProp) => {
  const { i18n } = useTranslate();
  const { t } = useTranslation('guestPage');

  const makeReservationOptions = useCallback(() => {
    if (pageInfo.confirmDetail.confirm) {
      return [
        {
          time: modalContentTime(
            toTzDateTime(
              pageInfo.confirmDetail.startDateTime,
              timezone
            ) as string,
            toTzDateTime(
              pageInfo.confirmDetail.endDateTime,
              timezone
            ) as string,
            i18n.language
          ),
      ...
      return pageInfo.confirmDetail.reservationOptions.map(
        ({ startDateTime, endDateTime }, idx) => ({
          time: modalContentTime(
            toTzDateTime(startDateTime, timezone),
            toTzDateTime(endDateTime, timezone),
            i18n.language
          ),
          priority: `${idx + 1}`,
        })
