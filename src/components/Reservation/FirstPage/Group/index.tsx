...
const GroupFirstPage = () => {
  ...
  const { t, i18n } = useTranslation('createGroupBookingPage');
  ...
  return (
    <>
      ...
      >
        {TermView(
          <>
            {pageInfo.term.custom.checked && groupAlert.endDate && (
              <ContentAlert>
                {t('alert.openDurationAlert1')} {groupEndDate || ''}{' '}
                {t('alert.openDurationAlert2')}
              </ContentAlert>
            )}
          </>
        )}
